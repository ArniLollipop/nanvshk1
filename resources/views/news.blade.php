        ...
        <div class="col-lg-4 col-md-6 col-sm-12 news-block">
            <div class="news-block-one wow fadeInUp animated" data-wow-delay="00ms" data-wow-duration="1500ms">
                <div class="inner-box p_relative d_block mb_70">
                    <div class="image-box p_relative d_block">
                        ...
                        <figure class="image p_relative d_block">
                            <a href="{{ route('news-details', ['info' => $info->slug, app()->getLocale()] ) }}"
                            class="img--change"
                            aria-label="Переход на {{ $info->title }}">
                ...
                <div class="lower-content p_relative d_block pt_25">
                    @if(app()->getLocale() == 'ru')
                        <h4 class="d_block fs_20 lh_30 mb_6 text--resize">
                            <a href="{{ route('news-details', ['info' => $info->slug, app()->getLocale()] ) }}" aria-label="Переход на {{ $info->title }}">
                                {{ $info->title }}
                            </a>
                        </h4>
                    @elseif(app()->getLocale() == 'kz')
                        <h4 class="d_block fs_20 lh_30 mb_6 text--resize">
                            <a href="{{ route('news-details', ['info' => $info->slug, app()->getLocale()] ) }}" aria-label="Переход на {{ $info->title_kz }}">
                                {{ $info->title_kz }}
                            </a>
                        </h4>
                    @else
                        <h4 class="d_block fs_20 lh_30 mb_6 text--resize">
                            <a href="{{ route('news-details', ['info' => $info->slug, app()->getLocale()] ) }}" aria-label="Переход на {{ $info->title_en }}">
                                {{ $info->title_en }}
                            </a>
                        </h4>
                    @endif
